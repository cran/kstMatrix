...
There is a certain overlap in fuctionality between the `kst`and `kstMatrix` packages, 
however the former uses a set representatin and the latter a matrix representation. The
...
### kmconstrDowling
The `kmconstrDowling` function applies the algorithm by Dowling (1993b) for computing
the closure under union of a family of sets. Please note that while this algorithm in 
general is very efficient this does not necessarily hold for its implementation in R 
since it involves nested for-loops.
```{r}
...
distance; the distance agreement coefficient is the ratio between the distance between
...
on reading and writing abilities (Dowling, 1991; 1993a). A forth basis represents
